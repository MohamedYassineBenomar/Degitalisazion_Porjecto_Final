...
**Gradient Boosting Regressor** (scikit-learn) instead of Prophet.
...
st.markdown(
    ...
    <div class="hm-hero">
        ...
        <p>Same demo, same data — pricing predictions by a Gradient Boosting Regressor (scikit-learn)</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with c1:
    st.markdown(
        ...
        <div class="hm-priceCard">
            ...
            <p class="sub">Generated by a Gradient Boosting Regressor — explicit date features, tree ensemble.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
st.caption(
    ...
    f"**Blue:** real 2017 ADR. **Purple:** what the Gradient Boosting "
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>With ML<small>Gradient Boosting,<br>elasticity-adjusted (η = {PRICE_ELASTICITY})</small></th>
    ...
    <div class="hm-chartKey">
        ...
        <ul>
            ...
            <li><span class="hm-swatch" style="background:#8e44ad;"></span>
                <b>ML forecast</b> — Gradient Boosting predictions for the next 90 days.</li>
...
if bookings_df.empty:
    ...
    st.caption(
        ...
        "bookings, **but re-priced with the Gradient Boosting model** "
...
with mc2:
    st.markdown("### 🟣 Gradient Boosting (scikit-learn)")
    ...
    st.caption(
        "Classical-ML approach: explicit date features fed into a tree "
        "ensemble. Handles non-linear interactions, can grow with new "
        "features (weather, events, competitor prices), but extrapolates "
        "trends conservatively and needs a hand-built uncertainty band."
    )

st.caption(
    "**Which is better?** On *this dataset*, Prophet wins on accuracy "
    "(lower MAPE), because its built-in seasonality model fits the "
    "August peak smoothly. The ML model is more conservative on the "
    "very-top peak days. **In a real production setting** where the "
    "hotel has weather data, local-event calendars, competitor rates, "
    "and channel-mix features, the ML approach scales more naturally "
    "— add features, retrain. For an MVP with just date + price, "
    "Prophet is the right choice; the ML page exists to show that the "
    "architecture is model-agnostic and we can swap predictors without "
